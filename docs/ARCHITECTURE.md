...
### Application
Depends on Domain and interface contracts. Contains MediatR Commands and Queries, FluentValidation validators, and interface definitions (`IPermissionService`, `IAuditLogger`, `IFileStorage`, `ISolverJobQueue`, `ISolverClient`, `INotificationService`, `IEmailSender`, `IInvitationSender`, `IPdfRenderer`, `IAiAssistant`). `AppDbContext` is defined under `Jobuler.Application/Persistence` to avoid circular project references - EF configurations are applied from the Infrastructure assembly at startup via `AppDbContext.ConfigurationAssembly`.
...
### API
Wires everything together via DI. Controllers dispatch commands/queries via MediatR - no direct repository calls. Contains middleware components including `ExceptionHandlingMiddleware` (global error to JSON response mapping), `TenantContextMiddleware` (sets PostgreSQL session variables for RLS), and ProblemDetails helpers.
...
### Frontend
Next.js App Router with server components for layout and locale resolution. Client components use TanStack Query for server data and Zustand for auth, admin session, connectivity, onboarding, theme, and active space state. API calls live in `lib/api`; the repo also keeps NSwag-generated OpenAPI assets under `lib/api/generated`.
...
1. Admin triggers `POST /spaces/{id}/schedule-runs/trigger` and receives `runId` immediately (202 Accepted)
...
7. If Redis is unavailable at startup, the API falls back to `InMemorySolverJobQueue` for single-server development.
...
| PostgreSQL | 5432 | PostgreSQL 16 | RLS enabled; `jobuler` user for app |
| Redis | 6379 | Redis | Solver job queue; app falls back to in-memory queue in single-server dev |
