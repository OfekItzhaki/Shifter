...
export default function SpaceSettingsPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaved(false);
    try {
      await updateSpace(currentSpaceId, {
        name: name.trim(),
    ...
    return (
      <AppShell>
        <div className="w-full max-w-md mx-auto py-16 text-center space-y-4">
          <p className="text-slate-500 dark:text-slate-400">{t("noSpace")}</p>
          <a
            href="/onboarding"
  ...
  return (
    <AppShell>
      <div
        style={{ maxWidth: 720, direction: isRtl ? "rtl" : "ltr" }}
        className="w-full"
      >
        {/* Header */}
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          >
            ...
            {t("groupCount", { count: space.groupCount })}
          </p>
        </div>

        <div
          role="tablist"
          aria-label={t("settingsTabs.label")}
          className="mb-4 flex gap-1 overflow-x-auto rounded-xl bg-slate-100 p-1 dark:bg-slate-800"
        >
          {(
            [
              "general",
            ...
            <button
              ...
              className={`min-h-[40px] whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                activeTab === tab
            ...
            <>
              ...
              <InviteCodeCard
                ...
                onCodeRegenerated={(newCode) =>
                  setSpace((prev) =>
                    prev ? { ...prev, inviteCode: newCode } : prev,
                  )
                }
              />
            </>
