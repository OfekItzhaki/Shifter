...
import { resolveTheme, useThemeStore } from "@/lib/store/themeStore";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 dark:text-white" dir={getLocaleDirection(lang)}>
      {/* Nav */}
      <nav className="sticky top-0 z-50 backdrop-blur-md bg-white/85 border-b border-slate-200/80 dark:bg-slate-900/80 dark:border-slate-700/50">
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-slate-200/80 px-6 py-4 space-y-3 bg-white/95 dark:border-slate-700/50 dark:bg-slate-900/95">
            <a href="#features" onClick={() => setMobileMenuOpen(false)} className="block text-sm text-slate-700 dark:text-slate-300">{c.nav.features}</a>
            <a href="#how-it-works" onClick={() => setMobileMenuOpen(false)} className="block text-sm text-slate-700 dark:text-slate-300">{c.nav.howItWorks}</a>
            <a href="#about" onClick={() => setMobileMenuOpen(false)} className="block text-sm text-slate-700 dark:text-slate-300">{c.nav.about}</a>
            <a href="#faq" onClick={() => setMobileMenuOpen(false)} className="block text-sm text-slate-700 dark:text-slate-300">{c.nav.faq}</a>
            <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="block text-sm text-sky-400 font-medium">{c.nav.signIn}</Link>
            <div className="flex gap-2 pt-2">
              {SUPPORTED_LOCALES.map(l => (
                <button key={l} onClick={() => { switchLang(l); setMobileMenuOpen(false); }} className={`text-xs px-3 py-1.5 rounded ${lang === l ? "bg-sky-100 text-sky-700 font-bold dark:bg-sky-500/30 dark:text-sky-300" : "text-slate-500 dark:text-slate-400"}`}>
                  {LOCALE_META[l].label}
                </button>
              ))}
            </div>
      ...
      <section className="px-6 pt-16 pb-20 sm:pt-24 sm:pb-28 max-w-4xl mx-auto text-center">
        ...
        <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto mb-10 leading-relaxed">{c.hero.subtitle}</p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <Link href="/login" className="w-full sm:w-auto text-center text-slate-700 hover:text-slate-950 border border-slate-300 hover:border-slate-400 px-8 py-4 rounded-2xl text-base transition-all dark:text-slate-300 dark:hover:text-white dark:border-slate-600 dark:hover:border-slate-400">{c.hero.signIn}</Link>
        </div>
      </section>
      ...
      <section id="features" className="px-6 py-20 max-w-5xl mx-auto scroll-mt-20">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-center mb-12 max-w-xl mx-auto">{c.features.subtitle}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {c.features.items.map(f => (
            <div key={f.title} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition-colors dark:bg-slate-800/60 dark:border-slate-700/50 dark:hover:border-slate-600">
      ...
      {/* How it works */}
      <section id="how-it-works" className="px-6 py-20 bg-white/70 scroll-mt-20 dark:bg-slate-800/50">
      ...
      <section id="about" className="px-6 py-20 max-w-4xl mx-auto scroll-mt-20">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-center mb-10 max-w-xl mx-auto">{c.about.subtitle}</p>
        <div className="bg-white border border-slate-200 rounded-2xl p-8 sm:p-10 space-y-5 text-slate-600 shadow-sm leading-relaxed dark:bg-slate-800/60 dark:border-slate-700/50 dark:text-slate-300">
          {c.about.paragraphs.map((p, i) => <p key={i}>{i === 0 ? <><strong className="text-slate-950 dark:text-white">Shifter</strong> {p.replace("Shifter ", "")}</> : p}</p>)}
        </div>
      ...
      {/* FAQ */}
      <section id="faq" className="px-6 py-20 bg-white/70 scroll-mt-20 dark:bg-slate-800/50">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4">{c.faq.title}</h2>
          <p className="text-slate-500 dark:text-slate-400 text-center mb-10">{c.faq.subtitle}</p>
          <div className="space-y-4">
            {c.faq.items.map(item => (
              <details key={item.q} className="group bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm dark:bg-slate-800/60 dark:border-slate-700/50">
                <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none">
                  <span className="text-sm font-medium text-slate-950 dark:text-white">{item.q}</span>
                ...
                <div className="px-6 pb-4 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{item.a}</div>
              </details>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="px-6 py-8 border-t border-slate-200/80 dark:border-slate-700/50">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <ShifterLogo size={20} />
            <span className="text-sm text-slate-500 dark:text-slate-400">Shifter &copy; {new Date().getFullYear()}</span>
          </div>
          <div className="flex items-center gap-6 text-sm text-slate-500 dark:text-slate-400">
            <a href="#about" className="hover:text-slate-950 dark:hover:text-white transition-colors">{c.footer.about}</a>
            <a href="#faq" className="hover:text-slate-950 dark:hover:text-white transition-colors">{c.footer.faq}</a>
            {LANDING_LEGAL_LINKS[lang].map(link => (
              <Link key={link.href} href={link.href} className="hover:text-slate-950 dark:hover:text-white transition-colors">
                {link.label}
              </Link>
            ))}
            <Link href="/login" className="hover:text-slate-950 dark:hover:text-white transition-colors">{c.footer.signIn}</Link>
          </div>
        </div>
      </footer>
    </div>
