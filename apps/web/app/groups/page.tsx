...
import { useGroups, useDeletedGroups, useCreateGroup, useRestoreGroup, type GroupDto } from "@/lib/query/hooks/useGroups";
...
function GroupsPage() {
  ...
  const visibleGroups = useMemo(
    () => groups.filter(group => (group.parentGroupId ?? null) === effectiveParentId),
  ...
  const breadcrumbs = useMemo(() => {
    ...
    while (cursor && !visited.has(cursor.id)) {
      ...
      cursor = cursor.parentGroupId ? byId.get(cursor.parentGroupId) ?? null : null;
    }
  ...
  async function handleCreateWithWizard(name: string, templateId: string, schedulingMode?: SchedulingMode) {
    ...
    setCreateError(null);
    try {
      const result = await createGroup.mutateAsync({ name, parentGroupId: effectiveParentId });
  ...
  return (
    <AppShell>
      <div className="max-w-4xl space-y-5 sm:space-y-6">
        ...
        <nav aria-label="Group hierarchy" className="flex flex-wrap items-center gap-1.5 text-xs sm:text-sm text-slate-500">
          <button
            type="button"
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 flex gap-2">
                    <button
                      type="button"
                      onClick={() => navigateToLevel(g.id)}
                      className="flex-1 rounded-xl bg-slate-900 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-slate-700"
                    >
                      {t("openGroup")}
                    </button>
                    <button
                      ...
                      className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-50"
                    >
                      {t("manageGroup")}
                    </button>
                  </div>
