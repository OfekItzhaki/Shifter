...
public class QuestPdfRenderer : IPdfRenderer
{
    ...
    public byte[] Render(SelfServiceCloseoutPdfModel model)
    {
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    col.Item().Text($"{model.SpaceName} / {model.GroupName} - Self-service closeout")
                        .FontSize(16).Bold();
                    col.Item().Text($"Cycle: {model.StartsAt:yyyy-MM-dd HH:mm} UTC - {model.EndsAt:yyyy-MM-dd HH:mm} UTC")
                ...
                {
                    col.Item().Background(Colors.Grey.Lighten4).Border(0.5f).BorderColor(Colors.Grey.Lighten1)
                        .Padding(8).Column(summary =>
                        {
                            summary.Item().Text("Verification fingerprint").Bold();
                            summary.Item().Text(model.ReportFingerprint).FontSize(7).FontColor(Colors.Grey.Darken2);
                        });
                    ...
                    col.Item().PaddingTop(12).Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            header.Cell().Element(HeaderCell).Text("Metric").FontColor(Colors.White).Bold();
                            header.Cell().Element(HeaderCell).Text("Value").FontColor(Colors.White).Bold();
                        });
                        ...
                        foreach (var metric in model.Metrics)
                        {
                            ...
                            static IContainer DataCell(IContainer c, string color) =>
                                c.Background(color).BorderBottom(0.5f).BorderColor(Colors.Grey.Lighten2).Padding(5);

                            table.Cell().Element(c => DataCell(c, bg)).Text(metric.Label);
                            table.Cell().Element(c => DataCell(c, bg)).Text(metric.Value);
                        }
                    });
                });
                ...
                page.Footer().AlignRight()
                    .Text(x =>
                    {
                        x.Span("Generated by Shifter - Page ").FontSize(8).FontColor(Colors.Grey.Medium);
                        x.CurrentPageNumber().FontSize(8).FontColor(Colors.Grey.Medium);
                        x.Span(" of ").FontSize(8).FontColor(Colors.Grey.Medium);
                        x.TotalPages().FontSize(8).FontColor(Colors.Grey.Medium);
                    });
