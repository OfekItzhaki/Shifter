    ...
    : IRequestHandler<ExportOrganizationPackageCommand, OrganizationExportPackageResult>
{
        ...
        CancellationToken ct)
    {
        ...
        var userIds = await _db.SpaceMemberships.AsNoTracking()
            ...
            .Union(_db.Spaces.AsNoTracking()
                .Where(s => spaceIds.Contains(s.Id))
                .Select(s => s.OwnerUserId))
            .Union(_db.Organizations.AsNoTracking()
                .Where(o => o.Id == request.OrganizationId)
                .Select(o => o.PrimaryOwnerUserId))
            .Union(_db.Groups.AsNoTracking()
                .Where(g => spaceIds.Contains(g.SpaceId) && g.CreatedByUserId.HasValue)
                .Select(g => g.CreatedByUserId!.Value))
            .Union(_db.GroupTasks.AsNoTracking()
                .Where(t => spaceIds.Contains(t.SpaceId) && t.CreatedByUserId.HasValue)
                .Select(t => t.CreatedByUserId!.Value))
            .Union(_db.GroupTasks.AsNoTracking()
                .Where(t => spaceIds.Contains(t.SpaceId) && t.UpdatedByUserId.HasValue)
                .Select(t => t.UpdatedByUserId!.Value))
            .Union(_db.TaskSlots.AsNoTracking()
                .Where(s => spaceIds.Contains(s.SpaceId) && s.CreatedByUserId.HasValue)
                .Select(s => s.CreatedByUserId!.Value))
            .Union(_db.ConstraintRules.AsNoTracking()
                .Where(c => spaceIds.Contains(c.SpaceId) && c.CreatedByUserId.HasValue)
                .Select(c => c.CreatedByUserId!.Value))
            .Union(_db.ConstraintRules.AsNoTracking()
                .Where(c => spaceIds.Contains(c.SpaceId) && c.UpdatedByUserId.HasValue)
                .Select(c => c.UpdatedByUserId!.Value))
            .Union(_db.ScheduleVersions.AsNoTracking()
                .Where(v => spaceIds.Contains(v.SpaceId) && v.CreatedByUserId.HasValue)
                .Select(v => v.CreatedByUserId!.Value))
            .Union(_db.ScheduleVersions.AsNoTracking()
                .Where(v => spaceIds.Contains(v.SpaceId) && v.PublishedByUserId.HasValue)
                .Select(v => v.PublishedByUserId!.Value))
            .Union(_db.SpecialLeaveRequests.AsNoTracking()
                .Where(r => spaceIds.Contains(r.SpaceId))
                .Select(r => r.RequestedByUserId))
            .Union(_db.SpecialLeaveRequests.AsNoTracking()
                .Where(r => spaceIds.Contains(r.SpaceId) && r.ProcessedByUserId.HasValue)
                .Select(r => r.ProcessedByUserId!.Value))
            .Union(_db.Notifications.AsNoTracking()
                .Where(n => spaceIds.Contains(n.SpaceId))
                .Select(n => n.UserId))
            .Union(_db.AuditLogs.AsNoTracking()
                .Where(l => l.SpaceId.HasValue && spaceIds.Contains(l.SpaceId.Value) && l.ActorUserId.HasValue)
