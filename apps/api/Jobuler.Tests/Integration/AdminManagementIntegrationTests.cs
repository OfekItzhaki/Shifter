...
public class AdminManagementIntegrationTests
{
    ...
    private static PublishVersionCommandHandler MakePublishHandler(AppDbContext db)
    {
        ...
        return new PublishVersionCommandHandler(db, NoOpAuditLogger(), config, logger, scopeFactory, snapshotService, cumulativeTracker, new Helpers.NoOpCacheService(), scheduleNotifications: null);
    }
    ...
    [Fact]
    public async Task PublishVersionCommand_BlocksSameGroupGeneratedAssignmentsWithInsufficientRest()
    {
        var db = CreateDb();
        var spaceId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        ...
        var person = Person.Create(spaceId, "יעקוב");
        ...
        var outpostTask = GroupTask.Create(
            spaceId,
            group.Id,
            "תל 7",
            solverStart.AddHours(5),
            solverStart.AddHours(9),
            shiftDurationMinutes: 240,
            requiredHeadcount: 1,
            burdenLevel: TaskBurdenLevel.Normal,
            allowsDoubleShift: false,
        ...
        db.GroupTasks.AddRange(guardTask, outpostTask);
        db.Assignments.AddRange(
            Assignment.Create(spaceId, version.Id, DeriveShiftGuid(guardTask.Id, 0), person.Id),
            Assignment.Create(spaceId, version.Id, DeriveShiftGuid(outpostTask.Id, 0), person.Id));
        await db.SaveChangesAsync();
        ...
        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*only 1.0h rest*Required: 8h*");
